# version 1.3: 
# added support for 3 camera views
...
class DroneAutopilotGUI:
    def __init__(self, root):
        ...
        # Camera objects
        self.caps = [None, None, None]  # Will hold the webcam capture objects
        self.camera_sources = [0, 0, 0]  # Default camera sources (modify as needed)
        self.camera_labels = []  # Will hold the labels for each camera feed
    ...
    def create_layout(self):
        ...
        self.view_button = ttk.Button(view_toggle_frame, text="Switch to Cameras", 
        ...
        # Multi-Camera view (initially hidden)
        self.camera_frame = ttk.Frame(self.center_frame)
        
        # Create frames for each camera
        camera_titles = ["Front Camera", "Bottom Camera", "Rear Camera"]
        
        # Top row with camera 1
        top_frame = ttk.Frame(self.camera_frame)
        top_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 5))
        
        cam1_frame = ttk.Frame(top_frame, borderwidth=2, relief="groove")
        cam1_frame.pack(fill=tk.BOTH, expand=True)
        
        ttk.Label(cam1_frame, text=camera_titles[0], font=("Arial", 10, "bold")).pack(anchor=tk.W, padx=5, pady=5)
        cam1_label = ttk.Label(cam1_frame)
        cam1_label.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.camera_labels.append(cam1_label)
        
        # Bottom row with cameras 2 and 3
        bottom_frame = ttk.Frame(self.camera_frame)
        bottom_frame.pack(fill=tk.BOTH, expand=True, pady=(5, 0))
        
        # Camera 2
        cam2_frame = ttk.Frame(bottom_frame, borderwidth=2, relief="groove")
        cam2_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 5))
        
        ttk.Label(cam2_frame, text=camera_titles[1], font=("Arial", 10, "bold")).pack(anchor=tk.W, padx=5, pady=5)
        cam2_label = ttk.Label(cam2_frame)
        cam2_label.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.camera_labels.append(cam2_label)
        
        # Camera 3
        cam3_frame = ttk.Frame(bottom_frame, borderwidth=2, relief="groove")
        cam3_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=(5, 0))
        
        ttk.Label(cam3_frame, text=camera_titles[2], font=("Arial", 10, "bold")).pack(anchor=tk.W, padx=5, pady=5)
        cam3_label = ttk.Label(cam3_frame)
        cam3_label.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.camera_labels.append(cam3_label)
        ...
        camera_settings_frame = ttk.Frame(telemetry_frame, padding=10)
        camera_settings_frame.pack(fill=tk.X, pady=5)
        camera_settings_frame.configure(style="Card.TFrame")
        
        ttk.Label(camera_settings_frame, text="Camera Settings", font=("Arial", 10, "bold"), 
                 foreground="#aaaaaa").pack(anchor=tk.W)
        
        camera_settings_grid = ttk.Frame(camera_settings_frame)
        camera_settings_grid.pack(fill=tk.X, pady=(5, 0))
        
        # Add camera recording buttons
        self.rec_buttons = []
        for i, name in enumerate(["Front", "Bottom", "Rear"]):
            ttk.Label(camera_settings_grid, text=f"{name}:").grid(row=i, column=0, sticky=tk.W, pady=2)
            rec_btn = ttk.Button(camera_settings_grid, text="REC", width=5, 
                                command=lambda idx=i: self.toggle_recording(idx))
            rec_btn.grid(row=i, column=1, sticky=tk.E)
    ...
    def toggle_recording(self, camera_idx):
        """Toggle recording for a specific camera"""
        # This is a placeholder for actual recording functionality
        button = self.rec_buttons[camera_idx]
        if button["text"] == "REC":
            button.configure(text="STOP", foreground="red")
        else:
            button.configure(text="REC", foreground="black")
    ...
    def toggle_view(self):
        ...
        if self.current_view == "map":
            ...
            self.view_button.configure(text="Switch to Cameras")
    ...
    def start_cameras(self):
        ...
        if not self.camera_active:
            ...
            # Try to open different camera sources for each camera
            # In a real drone system, these would be different camera devices
            # For demo purposes, we'll use the same webcam with different simulated effects
            for i in range(3):
                self.caps[i] = cv2.VideoCapture(self.camera_sources[i])
                if not self.caps[i].isOpened():
                    # If camera can't be opened, show error message
                    self.camera_labels[i].configure(text=f"Error: Could not open camera {i+1}")
            
            # Start updating camera feeds
            for i in range(3):
                self.update_camera(i)
    
    def update_camera(self, camera_idx):
        """Update specific camera feed"""
        if self.camera_active and self.caps[camera_idx] and self.caps[camera_idx].isOpened():
            ret, frame = self.caps[camera_idx].read()
            if ret:
                # Convert frame from BGR to RGB color space
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                
                # Apply different effects for each camera to simulate different camera positions
                if camera_idx == 0:  # Front camera (normal)
                    pass  # No effect for front camera
                elif camera_idx == 1:  # Bottom camera (slightly darker)
                    frame = cv2.convertScaleAbs(frame, alpha=0.8, beta=0)
                elif camera_idx == 2:  # Rear camera (mirrored)
                    frame = cv2.flip(frame, 1)
                
                # Add telemetry overlay to camera view
                self.add_camera_overlay(frame, camera_idx)
                
                # Convert to PhotoImage for Tkinter
                # Resize the image to fit the frame better
                height, width = frame.shape[:2]
                if camera_idx == 0:
                    # Front camera takes up more space
                    new_width = 640
                else:
                    # Side-by-side cameras are smaller
                    new_width = 320
                ...
                # Update the label
                self.camera_labels[camera_idx].imgtk = imgtk
                self.camera_labels[camera_idx].configure(image=imgtk)
                
            # Schedule the next update
            if self.camera_active:
                self.root.after(30, lambda: self.update_camera(camera_idx))
    
    def add_camera_overlay(self, frame, camera_idx):
        """Add compass overlay and telemetry data to camera frame"""
        height, width = frame.shape[:2]
        
        # Add camera-specific overlays
        camera_names = ["FRONT CAM", "BOTTOM CAM", "REAR CAM"]
        cv2.putText(frame, camera_names[camera_idx], (10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        # Add compass (only to front camera)
        if camera_idx == 0:
            center_x = 80
            center_y = 80
            radius = 40
        ...
        # Add telemetry data overlay
        # Common overlay for all cameras
        battery_color = (0, 255, 0) if self.battery_level > 20 else (0, 0, 255)
        cv2.putText(frame, f"Batt: {self.battery_level:.1f}%", (width - 150, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, battery_color, 1)
        
        # Different telemetry for each camera
        if camera_idx == 0:  # Front camera shows navigation data
            cv2.putText(frame, f"Alt: {self.altitude:.1f}m", (width - 150, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(frame, f"Spd: {self.speed:.1f}m/s", (width - 150, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(frame, f"Hdg: {self.heading}°", (width - 150, 120), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        elif camera_idx == 1:  # Bottom camera shows position data
            cv2.putText(frame, f"Lat: {self.lat:.4f}°", (width - 150, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(frame, f"Lon: {self.lon:.4f}°", (width - 150, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(frame, f"Alt: {self.altitude:.1f}m", (width - 150, 120), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        elif camera_idx == 2:  # Rear camera shows orientation data
            cv2.putText(frame, f"Roll: {self.roll:.1f}°", (width - 150, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(frame, f"Pitch: {self.pitch:.1f}°", (width - 150, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(frame, f"Yaw: {self.heading}°", (width - 150, 120), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        
        # Add recording indicator if recording
        if camera_idx < len(self.rec_buttons) and self.rec_buttons[camera_idx]["text"] == "STOP":
            # Add red recording indicator
            cv2.circle(frame, (20, height - 20), 10, (0, 0, 255), -1)
            cv2.putText(frame, "REC", (35, height - 15), 
